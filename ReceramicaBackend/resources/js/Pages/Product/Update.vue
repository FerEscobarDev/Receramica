...
const form = useForm({
    ...
    landing: Boolean(props.product.landing),
    images: props.product.images.map(image => ({ url: url + image.url, isMain: image.main, file: null }))
});

const selectedImages = ref(form.images);
...
const submit = () => {
    form.transform(data => ({
        ...data//,
        //images: data.images.map(image => image.file ? image.file : image.url)
    })).post(route('creaciones.update', props.product.id));
};
</script>

<template>
    <AppLayout title="Actualizar Creación">
        <template #header>
